    ...
    *shard_len = 0;
    if (filename != NULL)
    {
        ...
        while ((read = getline(&line, &len, fhandler)) != -1)
        {
            ...
            if (*shard_len >= 2)
            {
                Shard *shards_array_temp = realloc(shards_array, (*shard_len) * sizeof(Shard));
    ...
    // fprintf(stderr,">>>>%s,%s,%s,%d\n",
    //     shards_array[0].giggle_path, shards_array[0].stixdb_path, shards_array[0].bed_path,*shard_len);
    return shards_array;
}
    ...
    *table_len = 0;
    if (filename != NULL)
    {
        ...
        while ((read = getline(&line, &len, fhandler)) != -1)
        {
            ...
            if (*table_len >= 2)
            {
                TableQuery *tablequery_array_temp = realloc(tablequery_array, (*table_len) * sizeof(TableQuery));
                ...
                case 0:
                    ...
                    else
                    {
                        if (!tablequery.left_str)
                        {
                            ...
                            free(tablequery.ID);
                            errx(EX_TEMPFAIL, "ERROR: Memory allocation failed for left_str.");
                        }
                    }
                    break;
                case 1:
