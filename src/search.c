...
struct stix_breakpoint *stix_region_to_breakpoint(char *region) {
    struct stix_breakpoint *bp =
        (struct stix_breakpoint *)calloc(1, sizeof(struct stix_breakpoint));

    if (parse_region(region, &(bp->chrm), &(bp->start), &(bp->end)) != 0)
        errx(EX_USAGE, "Error parsing region '%s'\n", region);
...
uint32_t stix_parse_result(
    ...
    if (*left == NULL) {
        *left = (struct stix_breakpoint *)malloc(sizeof(struct stix_breakpoint));
        (*left)->chrm = NULL;
    }

    if (*right == NULL) {
        *right = (struct stix_breakpoint *)malloc(sizeof(struct stix_breakpoint));
        (*right)->chrm = NULL;
    }
...
uint32_t stix_check_sv(
    ...
    switch (sv_type) {
    case DEL:
        return stix_check_del(
            q_left_bp, q_right_bp, in_left_bp, in_right_bp, evidence_type, slop
        );
        break;
    case DUP:
        return stix_check_dup(
            q_left_bp, q_right_bp, in_left_bp, in_right_bp, evidence_type, slop
        );
        break;
    case INV:
        return stix_check_inv(
            q_left_bp, q_right_bp, in_left_bp, in_right_bp, evidence_type, slop
        );
        break;
    case INS:
...
uint32_t stix_check_del(
    ...
    if (evidence_type == 0) { // paired-end
        if ((in_left_bp->strand != 1) || (in_right_bp->strand != -1))
            return 0;
    } else { // ignore the insertion region which the left-end = right-start (0bp
             // region)
        if (in_left_bp->strand != in_right_bp->strand)
            return 0;
    }
    ...
    float ov;
    if (in_left_bp->start < in_right_bp->start) { // a is head of b
        if (in_left_bp->end < in_right_bp->start) {
            // no overlap
            ov = -1.0;
            if (V_is_set) {
                fprintf(
                    stderr,
                    "#overlap: ov:%f,min_len:%f, in_left_bp->start: %d, "
                    "in_right_bp->start: %d, in_left_bp->end: %d, in_right_bp->end: "
                    "%d\n",
                    ov,
                    min_len,
                    in_left_bp->start,
        ...
        // b is head of a
        if (in_right_bp->end < in_left_bp->start) { // no overlap
            ov = -1.0;
            if (V_is_set) {
                fprintf(
                    stderr,
    ...
    if (V_is_set) {
        fprintf(
            stderr,
            "#overlap: ov:%f,min_len:%f, in_left_bp->start: %d, in_right_bp->start: "
            "%d, in_left_bp->end: %d, in_right_bp->end: %d\n",
            ov,
            min_len,
            in_left_bp->start,
            in_right_bp->start,
...
uint32_t stix_check_ins(
    ...
    */
    ...
    // Make sure the interval is a insertion record: right_interval.start ==
    // left_interval.end
    ...
    if (in_right_bp->start != in_left_bp->end) {
        ...
        */
        ...
        if (q_left_bp->end == in_right_bp->start) {
            ...
            // further compare length. beacuse multiple insertions exists in same insert
            // point.

            if (in_right_bp->start != in_right_bp->end &&
                q_right_bp->end !=
                    q_right_bp->start) { // ** in this case, length information were
                                         // encoded in right region
            ...
            /*
                If the results does not match query exactly. Then
                    ...
                    If the in_right_bp->start != in_right_bp-> end ==> Length encoded in
               the in_right_bp compare the length Else ==> No length information encoded
               in the in_right_bp check if the position of result with in the padding
               region of query position

            */

            // ** in this case, check distance of insert points between query and
            // result. **
            ...
            if (distance > slop) {
                if (V_is_set) {
                    fprintf(
                        stderr,
                        "#ins debug: found distance of quey: %d  and results:%d are "
                        "more than slop %d \n",
                        q_left_bp->end,
                        in_left_bp->end,
                        slop
            ...
            if (in_right_bp->start != in_right_bp->end &&
                ...
                if (V_is_set == 1) {
                    fprintf(
                        stderr,
                        "#ins debug: \n#  in_right_bp->end: %d, in_right_bp->start: "
                        "%d, results_len: %d\n#  q_right_bp->end: %d, "
                        "q_right_bp->start: %d, query_len: %d  \n#  pct: %f\n",
                        in_right_bp->end,
                        in_right_bp->start,
                ...
                // encoding length of right interval of query region as the length of
                // insertion if the results interval located in the query point
                // plus/minus the pad value and if the length of the query insertion and
                // results insertion is compatible(tolerate some difference) the result
                // interval will be reported as a true hit.
                if (((q_right_bp->start - ins_padding) < in_right_bp->start
                    ) && // end after start
                    ((q_right_bp->start + ins_padding) > in_right_bp->start
                    )) // start before end
                {
    ...
    */
    ...
    if ((in_left_bp->end >= q_left_bp->start) &&          // l end after start
        ...
        (in_right_bp->start < q_right_bp->end
           ...
           //(safe_sub(in_right_bp->start,slop) < q_right_bp->end) )   // r start before
           // end
        return 1;
    else
        return 0;
...
uint32_t stix_run_giggle_query(
    ...
    if (*gi == NULL) {
        *gi = giggle_load(giggle_index_dir, block_store_giggle_set_data_handler);
        if (*gi == NULL)
            errx(
                1,
    ...
    struct giggle_query_result *gqr =
        giggle_query(*gi, q_left_bp->chrm, q_start, q_end, NULL);
    ...
    if (*sample_alt_depths == NULL)
        *sample_alt_depths = (struct uint_pair *)malloc(N * sizeof(struct uint_pair));
    ...
    for (i = 0; i < N; i++) {
        ...
        while (giggle_query_next(gqi, &result) == 0) {
            ...
            uint32_t ret =
                stix_parse_result(result, &in_left_bp, &in_right_bp, &evidence_type);

            uint32_t hit = stix_check_sv(
                q_left_bp,
                q_right_bp,
                in_left_bp,
                in_right_bp,
            ...
            if (hit == 1) {
                ...
                if (V_is_set == 1) {
                    fprintf(
                        stderr,
                        "#debug: "
                        "query_left:%s %u %u\tresult_left:%s %u %u\tquery_right:%s %u "
                        "%u\tresult_right:%s %u %u\t%s\n",
                        q_left_bp->chrm,
                        q_left_bp->start,
                        q_left_bp->end,
...
uint32_t stix_get_quartile_counts(
    ...
    if (num_uniq >= 3) {
        ...
        *Q3 = uniq[MIN(num_uniq / 2 + 1 + num_uniq / 4, num_uniq - 1)];
        ...
        counts[1] = stix_bsearch_seq(*Q2, full, num_full, -1, num_full) - counts[0];

        counts[2] =
            stix_bsearch_seq(*Q3, full, num_full, -1, num_full) - counts[1] - counts[0];
...
uint32_t stix_get_summary(
    ...
    *max = 0;
    ...
    for (i = 0; i < num_samples; ++i) {
        ...
        /*from xinchang
        sample_alt_depths counts the number of evidence == 0 and number of evidence ==1
        which is in the seventh column of excord output evidence == 0 ==> paird reads
        evidence evidence == 1 ==> split reads evidence if you do a sum of
        sample_alt_depths, you are counting the number of split reads evidence
        */
        sum = sample_alt_depths[idx].first + sample_alt_depths[idx].second;
        // if (sum == 0)
        if (sum < min_supporting_reads)
            *zero_count = *zero_count + 1;
        // else if (sum >= 1) /* What happens if there are more than 1 split reads
        // evidence? */
        else if (sum >= min_supporting_reads) /* What happens if there are more than 1
    ...
    uint32_t ret = stix_get_quartile_counts(full, full_i, Q1, Q2, Q3, counts);
    free(full);
    return 0;
}
//}}}
...
/*
from xinchang
In sharding mode we should not include the filter function,beacuse index are start from
0 in each shard.
*/
...
uint32_t stix_get_summary_shard(
    ...
    *max = 0;
    ...
    // uint32_t  num_samples_size = sizeof(num_samples_all) /
    // sizeof(num_samples_all[0]);
    ...
    for (size_t num_sample_idx = 0; num_sample_idx < num_samples_size;
         num_sample_idx++) {
        ...
        for (i = 0; i < num_samples; ++i) {
            ...
            /*from xinchang
            sample_alt_depths counts the number of evidence == 0 and number of evidence
            ==1 which is in the seventh column of excord output evidence == 0 ==> paird
            reads evidence evidence == 1 ==> split reads evidence if you do a sum of
            sample_alt_depths, you are counting the number of split reads evidence
            */
            sum = sample_alt_depths_all[num_sample_idx][idx].first +
                  sample_alt_depths_all[num_sample_idx][idx].second;
            // if (sum == 0)
            //     *zero_count = *zero_count + 1;
            // else if (sum >= 1) /* What happens if there are more than 1 split reads
            // evidence? */
            ...
            // else if (sum >= 1) /* What happens if there are more than 1 split reads
            // evidence? */
            else if (sum >= min_supporting_reads) /* What happens if there are more than
                                                     1 split reads evidence? */
                *one_count = *one_count + 1;
            else {
                full[full_i] = sum;
                full_i += 1;
...
uint32_t stix_get_sample_depths(
    ...
    *sample_depths = (uint32_t *)calloc(num_samples, sizeof(uint32_t));
    ...
    for (i = 0; i < num_samples; ++i) {
        ...
        (*sample_depths)[i] =
            sample_alt_depths[idx].first + sample_alt_depths[idx].second;
    }
...
uint32_t stix_get_vcf_breakpoints(
    ...
    ret = bcf_get_info_string(hdr, line, "SVTYPE", &sv_type_str, &sv_type_len);
    ...
    ret = bcf_get_info_int32(hdr, line, "END", &end, &end_size);
    ...
    */
    ...
    ret_svlen = bcf_get_info_int32(hdr, line, "SVLEN", &sv_len, &sv_len_size);
    ...
    ret = bcf_get_info_int32(hdr, line, "CIPOS", &cipos, &ci_size);
    ...
    */
    ...
    ret = bcf_get_info_int32(hdr, line, "CIEND", &ciend, &ci_size);
    if (ret != -1) {
        if (strcmp(sv_type_str, "INS") != 0) {
            right->start += ciend[0];
            right->end += ciend[1];
